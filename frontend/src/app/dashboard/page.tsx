            ...
            {!isMobile && (
                <div style={{
                    ...
                    <div style={{ height: '44px', backgroundColor: colors.headerBg, borderBottom: `1px solid ${colors.mailListBorder}`, display: 'flex', alignItems: 'center', padding: '0 16px', gap: '8px', flexShrink: 0 }}>
                        ...
                        <div style={{ width: '1px', height: '20px', backgroundColor: colors.mailListBorder, margin: '0 4px' }} />
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <button onClick={() => selectedMail && handleReplyMail(selectedMail)} disabled={!selectedMail} title="Yanıtla" style={{ background: `linear-gradient(90deg, ${colors.accent}, #c06800)`, color: 'white', border: 'none', padding: '5px 14px', borderRadius: '4px', fontWeight: 700, cursor: !selectedMail ? 'default' : 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', opacity: !selectedMail ? 0.3 : 1 }}><Reply size={13} /> Yanıtla</button>
                            <button disabled={!selectedMail} title="İlet" style={{ background: `linear-gradient(90deg, ${colors.accent}, #c06800)`, color: 'white', border: 'none', padding: '5px 14px', borderRadius: '4px', fontWeight: 700, cursor: !selectedMail ? 'default' : 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', opacity: !selectedMail ? 0.3 : 1 }}><ArrowRight size={13} /> İlet</button>
                            <button onClick={() => { const uids = selectedUids.length > 0 ? selectedUids.join(',') : selectedMail?.uid; if (uids) handleArchiveMail(uids); }} disabled={!(selectedMail || selectedUids.length > 0)} title="Arşivle" style={{ background: `linear-gradient(90deg, ${colors.accent}, #c06800)`, color: 'white', border: 'none', padding: '5px 14px', borderRadius: '4px', fontWeight: 700, cursor: !(selectedMail || selectedUids.length > 0) ? 'default' : 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', opacity: !(selectedMail || selectedUids.length > 0) ? 0.3 : 1 }}><Archive size={13} /> Arşivle</button>
                            <button onClick={() => { const uids = selectedUids.length > 0 ? selectedUids.join(',') : selectedMail?.uid; if (uids) handleMarkAsSpam(uids); }} disabled={!(selectedMail || selectedUids.length > 0)} title="Spam" style={{ background: `linear-gradient(90deg, ${colors.accent}, #c06800)`, color: 'white', border: 'none', padding: '5px 14px', borderRadius: '4px', fontWeight: 700, cursor: !(selectedMail || selectedUids.length > 0) ? 'default' : 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', opacity: !(selectedMail || selectedUids.length > 0) ? 0.3 : 1 }}><AlertOctagon size={13} /> Spam</button>
                            <button onClick={() => { const uids = selectedUids.length > 0 ? selectedUids.join(',') : selectedMail?.uid; if (uids) handleDeleteMail(uids); }} disabled={!(selectedMail || selectedUids.length > 0)} title="Sil" style={{ background: `linear-gradient(90deg, ${colors.accent}, #c06800)`, color: 'white', border: 'none', padding: '5px 14px', borderRadius: '4px', fontWeight: 700, cursor: !(selectedMail || selectedUids.length > 0) ? 'default' : 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', opacity: !(selectedMail || selectedUids.length > 0) ? 0.3 : 1 }}><Trash2 size={13} /> Sil</button>
                        </div>
                        <div style={{ width: '1px', height: '20px', backgroundColor: colors.mailListBorder, margin: '0 4px' }} />
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <button onClick={() => handleNavigate('prev')} disabled={!selectedMail || filteredMails.findIndex(m => m.uid === selectedMail.uid) <= 0} title="Önceki" style={{ background: `linear-gradient(90deg, ${colors.accent}, #c06800)`, color: 'white', border: 'none', padding: '5px 14px', borderRadius: '4px', fontWeight: 700, cursor: (!selectedMail || filteredMails.findIndex(m => m.uid === selectedMail.uid) <= 0) ? 'default' : 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', opacity: (!selectedMail || filteredMails.findIndex(m => m.uid === selectedMail.uid) <= 0) ? 0.3 : 1 }}><ChevronLeft size={13} /> Önceki</button>
                            <button onClick={() => handleNavigate('next')} disabled={!selectedMail || filteredMails.findIndex(m => m.uid === selectedMail.uid) === (filteredMails.length - 1)} title="Sonraki" style={{ background: `linear-gradient(90deg, ${colors.accent}, #c06800)`, color: 'white', border: 'none', padding: '5px 14px', borderRadius: '4px', fontWeight: 700, cursor: (!selectedMail || filteredMails.findIndex(m => m.uid === selectedMail.uid) === (filteredMails.length - 1)) ? 'default' : 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', opacity: (!selectedMail || filteredMails.findIndex(m => m.uid === selectedMail.uid) === (filteredMails.length - 1)) ? 0.3 : 1 }}>Sonraki <ChevronRight size={13} /></button>
                        </div>
                    </div>
